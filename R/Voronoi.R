...
#' @param max.range       The max range when clustering; grids beyond this value will be clustered as "Outlier"
...
WVDpreprocess <- function(mds.mat,
  ...
  WVD.centroids.weight <- apply(spatial.distribution, 2, function(x)(sum(x > 0)))
  WVD.centroids.weight <- WVD.centroids.weight^centroid.weight
  WVD.centroids$Weight <-  WVD.centroids.weight
  ...
  grid_distance_cluster <- clusterGrid(grid_distance, max.range = max.range)
...
weightedVoronoiPlot <- function(centroids,
  ...
  set.seed(1111)
  if("Background" %in% grid_clusters$Cluster){
    col_palatte <- c(c24[1:nrow(centroids)], "gray95")
    names(col_palatte) <- c(centroids$Class, "Background")
  }else{
    col_palatte <- c24[1:nrow(centroids)]
  }
  p1 <- ggplot() +
...
clusterGrid <- function(distance.grid,
  ...
  res <- apply(distance.grid, 1, function(x){
    tmp <- which.min(x)
    if(min(x) > max.range) return(c(x[tmp], "Background"))
    else return(c(x[tmp], colnames(distance.grid)[tmp]))
  })
